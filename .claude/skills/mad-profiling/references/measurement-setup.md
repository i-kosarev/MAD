...
| rocprofv3 sessions | rank 0 | rank 1 | rank 2 | rank 3 | total |
|---|---:|---:|---:|---:|---:|
| attaches, counted as `tool initialization` lines | 580 | 26 | 31 | 31 | **668** |

The commands behind them are shell helpers -- `date`, `sleep`, `tee`, `cut`, `awk` and non-server
`python3` -- but the logs record only the attach, not what it attached to, so no per-command count
is given here. An earlier revision of this file carried one; its rows summed to 1,239 against a
total of 668 and it is gone rather than corrected, since nothing in the artifacts can reproduce it.
