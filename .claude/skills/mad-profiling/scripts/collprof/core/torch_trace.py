...
#: The whole scalar, up to the delimiter, not a leading run of digits. `([\d.]+)` was unanchored,
#: so `1e3` parsed as 1 and `12x` as 12 -- a biased duration that never reached `malformed` and
#: could then be cached as a complete parse.
RE_TRACE_DUR = re.compile(r'"ts":\s*[\d.eE+-]+,\s*"dur":\s*([^,}\]\s]+)')
...
def _until_truncated(fh, trace: Path, truncated: list):
    ...
    One file per rank, so one bad file must not cost the whole capture. A missing gzip trailer
    (``EOFError``), corrupt bytes mid-stream (``zlib.error``) and bytes that are not text
    (``UnicodeDecodeError``) are caught: read with ``errors="ignore"`` the last of those deleted
    the bad byte instead, so `1\xff2` arrived as `12` -- a fabricated value that passed the
    malformed check and could be cached as a complete parse.
    """
    try:
        yield from fh
                ...
                if '"dur"' in line:
                    ...
                    if dur:
                        try:
                            ...
                            # `float` accepts NaN and Infinity, which then poison every sum they
                            # reach while the parse still counts as clean and cacheable.
                            if not math.isfinite(last_dur):
                                raise ValueError(dur.group(1))
                        except ValueError:
