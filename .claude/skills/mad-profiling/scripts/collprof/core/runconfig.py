...
#: How much of a path-valued setting identifies what it points at. One component was too little:
#: `/models/Kimi/checkpoint` and `/models/DeepSeek/checkpoint` both end in `checkpoint`, and two
#: different models were reported as no difference at all. Two keeps a remount quiet -- that
#: changes the root, not the layout under it -- and errs towards reporting a difference, which is
#: the safe direction: a spurious row is read by a human, a missed one is not.
PATH_COMPONENTS = 2


def normalise(key: str, value, path_valued: frozenset) -> str | None:
    """A value comparable across runs: a path-valued setting by its last components only."""
    if value is None or key not in path_valued:
        return value
    return "/".join(str(value).rstrip("/").rsplit("/", PATH_COMPONENTS)[-PATH_COMPONENTS:])
