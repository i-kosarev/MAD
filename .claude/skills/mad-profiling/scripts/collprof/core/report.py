...
def sanitise_cell(value):
    """A cell that cannot become a formula when the artifact is opened in a spreadsheet.

    Values here come from log text a run controls, and a shared CSV or workbook is opened by
    people who did not produce it. Numbers pass through -- a negative number must stay one -- so
    only non-numeric strings with a formula lead are prefixed with an apostrophe.
    """
    if not isinstance(value, str) or not value.startswith(FORMULA_LEADS):
        return value
    try:
        float(value)
    except ValueError:
...
class CsvSink:
    ...
    def write(self, name: str, header: list, rows: list) -> None:
        ...
        with path.open("w", newline="") as fh:
            ...
            writer.writerow([sanitise_cell(c) for c in header])
            writer.writerows([sanitise_cell(c) for c in row] for row in rows)
...
def section_header(view: PhaseView, ctx: ReportContext) -> list:
    ...
    # Only when a collective parsed. These three describe what the RCCL channel saw, and on a
    # tuned run it saw nothing: empty `per_rank` and `writers` still rendered "one file per
    # process" and "nranks=1", which the same report then says was never read.
    if phase.sizes:
        lines += [
        ...
        lines.append("RCCL records: **none parsed for this phase**, so rank coverage and "
                     "communicator size are not known here")
    if ctx.command:
...
def section_counters(view: PhaseView, ctx: ReportContext) -> list:
    ...
    if not sampled:
        ...
        # The notes still run. Withholding the table is not a reason to withhold why: a sampler
        # that reported failure, rows lost to the filesystem and nodes with no file at all are
        # most worth saying exactly when nothing could be measured.
        return lines + _counter_notes(view, ctx, series, sorted(series))
...
def _counter_notes(view: PhaseView, ctx: ReportContext, series: dict, omitted: list) -> list:
    ...
    declared = {name for name, _pattern in ctx.spec.counters.kinds}
    unclassified = sorted({k for s in series.values()
                           for k in by_kind(s, ctx.spec.counters)} - declared)
    if unclassified:
        lines += ["", f"**Not classified by this engine: {', '.join(unclassified)}.** Shown as "
                      "reported, but a sysfs name can be a gauge or a timer as readily as a "
                      "counter, so a difference over one is not necessarily work done. The "
                      "comparison leaves them out for that reason."]
