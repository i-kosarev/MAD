...
class RunConfigLayout:
    ...
    #: Settings whose value is a path to the same thing under a different mount. Compared by the
    #: last `runconfig.PATH_COMPONENTS` components, so a remount is not a difference while two
    #: models under a shared leaf name -- `Kimi/checkpoint` against `DeepSeek/checkpoint` -- are.
