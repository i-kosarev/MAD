...
EXCLUDED_FROM_DIFF = ("ports, hosts and seeds excluded, and model and tokenizer paths compared "
                      "by their final two components rather than in full")
...
def build_counters(left: Arm, right: Arm, spec: EngineSpec) -> tuple:
    ...
    # Only what the engine classified. An unclassified sysfs entry keeps its own name and may be
    # a gauge or a timer as easily as a counter -- `active_rc_qp` is a queue-pair count -- and
    # differencing one reports a gauge's rise as traffic, while a fall is dropped as a wrap.
    # Named below the table instead, as something reported rather than compared.
    declared = {name for name, _pattern in spec.counters.kinds}
    shared = seen[left.name] & seen[right.name] & declared
...
def section_counters(left: Arm, right: Arm, spec: EngineSpec) -> list:
    ...
    lines = ["", "## What crossed the fabric", "",
             "Adapter counters, summed over each arm's nodes. **Totals, not rates**: the arms "
             "were asserted to have served the same requests, which is what makes the totals "
             "comparable at all -- dividing by each arm's wall clock would turn "
             ...
             "exchange rather than measuring it. **Equal request work is not sole-cause**: what "
             "else differs between the arms is in the configuration section above, and a "
             "difference here belongs to the backend only as far as that section allows.", "",
    ...
    declared = {name for name, _pattern in spec.counters.kinds}
    unclassified = sorted((counter_kinds_seen(left, spec) | counter_kinds_seen(right, spec))
                          - declared)
    if unclassified:
        lines += ["", f"**Reported but not compared: {', '.join(unclassified)}.** This engine has "
                      "not said what these entries mean, and a sysfs name can be a gauge or a "
                      "timer as readily as a counter. Differencing one would report a gauge's "
                      "rise as traffic and drop its fall as a wrap, so they are listed here and "
                      "left out of the table."]

    # Declared kinds only, so an unclassified entry is explained once, by the note above.
    ...
    lines += ["", "These are **verb counts, not causality**. A reply can itself be an RDMA write "
                  ...
                  "different amounts of it on the wire."]
    return lines
