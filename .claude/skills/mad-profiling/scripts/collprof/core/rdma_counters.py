...
A sampler writes ``epoch_ns,host,device,port,counter,value`` rows per node. Counts are per
adapter and cover every user of the NIC, so a total is a ceiling; only the difference between
two arms serving the same requests is comparable.
"""
