...
def test_a_tuned_report_does_not_describe_an_rccl_channel_it_never_read(tmp_path: Path):
    """With no collectives the header still claimed a file layout and `nranks=1`, contradicting
    the same report's note that the channel did not report."""
    run = tmp_path / "tuned"
    write(run / "decode_NODE2.log",
          [server_args_line(), decode_batch_line(batch=16, rate=100.0), topo_line()])

    out = tmp_path / "out_decode"
    phase = parse_run(run, engines.get("sglang-disagg"))["decode"]
    assert not phase.sizes
    emit_phase(phase, out, ReportContext(spec=engines.get("sglang-disagg"), run_dir=run))

    text = (out / "report.md").read_text()
    assert "one file per process" not in text
    assert "nranks=" not in text
    assert "Ranks present in the log" not in text
    assert "**none parsed for this phase**" in text


def test_a_run_with_collectives_still_describes_the_rccl_channel(sglang_run: Path,
                                                                 tmp_path: Path):
    """The other half of the rule: gating those lines on `phase.sizes` must not drop them from
    the reports that did read the channel."""
    text = build(sglang_run, tmp_path / "out")["decode"]

    assert "Ranks present in the log:" in text
    assert "RCCL records read from:" in text
    assert "nranks=" in text
    assert "none parsed for this phase" not in text
...
def test_a_setting_that_looks_like_a_formula_is_not_one_in_the_csv(tmp_path: Path):
    """Values come from log text a run controls and the CSV is opened by people who did not
    produce it, so a leading `=` must not execute when the artifact is shared."""
    from collprof.core.report import CsvSink, sanitise_cell

    sink = CsvSink(tmp_path)
    sink.write("t.csv", ["key", "value"],
               [["served_model_name", "=cmd|' /c calc'!A1"], ["batch", "-64"],
                ["note", "+x"], ["plain", "kimi"]])

    rows = (tmp_path / "t.csv").read_text().splitlines()

    assert rows[1].startswith("served_model_name,'="), "the formula lead is escaped"
    assert rows[2] == "batch,-64", "a negative number stays a number"
    assert rows[3] == "note,'+x"
    assert rows[4] == "plain,kimi"
    assert sanitise_cell(3.5) == 3.5, "non-strings pass through untouched"


def test_a_prose_line_cannot_become_a_workbook_formula(tmp_path: Path):
    """`--left-name` is the caller's string and reaches the report title, so the prose rows need
    the same escaping the table cells got."""
    import importlib
    ...
    from collprof.core.workbook import write_workbook

    out = write_workbook(tmp_path, ["# =1+1 against DeepEP", "", "plain paragraph"])
    assert out is not None

    values = [c.value for row in openpyxl.load_workbook(out).worksheets[0].iter_rows()
              for c in row]
    assert any(isinstance(v, str) and v.startswith("'=1+1") for v in values), values
