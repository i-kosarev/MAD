...
class TestDiff:
    ...
    def test_the_same_weights_at_another_mount_is_not_a_difference(self):
        ...
        A remount changes the root, not the layout under it, which is what the comparison keeps.
        """
        a = {"model_path": "/mnt/weights/Kimi-K2-Instruct"}
        b = {"model_path": "/shared/weights/Kimi-K2-Instruct"}
        assert diff_configs(a, b, perf_relevant=CFG.perf_relevant, noise=CFG.noise,
                            path_valued=CFG.path_valued) == []

        other = {"model_path": "/mnt/weights/DeepSeek-R1"}
        (setting,) = diff_configs(a, other, perf_relevant=CFG.perf_relevant, noise=CFG.noise,
                                  path_valued=CFG.path_valued)
        assert setting.key == "model_path"
        assert (setting.left, setting.right) == ("weights/Kimi-K2-Instruct",
                                                 "weights/DeepSeek-R1")
        # And marked, not merely listed.
        assert setting.perf_relevant, "a different model is not a throughput-neutral difference"

    def test_two_models_under_a_generic_leaf_are_not_the_same_weights(self):
        """One component was too little: both of these end in `checkpoint`, so two different
        models were reported as no configuration difference at all."""
        kimi = {"model_path": "/models/Kimi-K2-Instruct/checkpoint"}
        deepseek = {"model_path": "/models/DeepSeek-R1/checkpoint"}

        (setting,) = diff_configs(kimi, deepseek, perf_relevant=CFG.perf_relevant,
                                  noise=CFG.noise, path_valued=CFG.path_valued)

        assert setting.key == "model_path"
        assert (setting.left, setting.right) == ("Kimi-K2-Instruct/checkpoint",
