...
class TestSampler:
    ...
    def test_a_counter_that_stops_reading_marks_the_window(self, tmp_path: Path):
        """A counter readable at the baseline and not later shortens the window without
        shortening the file: the parser still sees many samples and reports the delta as whole."""
        root = self.fake_sysfs(tmp_path / "sys", {"rx_write_req": 10})
        counter = root / "bnxt_re0" / "ports" / "1" / "hw_counters" / "rx_write_req"
        out = tmp_path / "decode_NODE2.csv"
        ...
        try:
            for _ in range(100):
                if (tmp_path / "decode_NODE2.csv.started").exists():
                    break
                time.sleep(0.05)
            counter.write_text("not a number\n")
            for _ in range(100):
                if (tmp_path / "decode_NODE2.csv.failed").exists():
                    break
                time.sleep(0.05)
        finally:
            proc.terminate()
        ...
        marker = tmp_path / "decode_NODE2.csv.failed"
        assert marker.exists(), "the shortened window is marked"
        assert "lost" in marker.read_text()
        assert parse_counters([out], LAYOUT)["decode_NODE2"].sampler_failed

    def test_a_baseline_that_read_nothing_is_marked(self, tmp_path: Path):
        """Counter files that exist but never yield a number would otherwise have an empty
        baseline declared complete by the `.started` marker."""
        root = tmp_path / "sys"
        port = root / "bnxt_re0" / "ports" / "1" / "hw_counters"
        port.mkdir(parents=True)
        (port / "weird").write_text("not a number\n")
        out = tmp_path / "decode_NODE2.csv"
        ...
        assert (tmp_path / "decode_NODE2.csv.failed").exists()
...
def test_a_withheld_table_still_says_what_went_wrong(sglang_run: Path, tmp_path: Path):
    """Every series sampled once withholds the table, and the diagnostics used to go with it --
    exactly when a reader most needs to know a sampler failed or a node was never sampled."""
    from collprof.cli import load_counters
    from collprof.core.rccl_log import parse_run
    from collprof.core.report import ReportContext, emit_phase

    # One decode node sampled once, its sampler marked failed; the other never sampled at all.
    write(sglang_run / "rdma" / "decode_NODE2.csv",
          sampled(["1000000000,mlx5_0,1,rx_write_req,5"]))
    (sglang_run / "rdma" / "decode_NODE2.csv.failed").write_text("sampler exited 4\n")
    ...
    text = (out / "report.md").read_text()
    assert "no window to take a difference over" in text, "the table is still withheld"
    assert "could not finish" in text, "and the sampler failure is still reported"
    assert "decode_NODE3" in text.split("What crossed the fabric")[1], "as is the unsampled node"
...
class TestUnequalCoverageWithholds:
    ...
    def test_the_table_does_not_claim_the_backend_is_the_only_difference(self, tmp_path: Path):
        """`--counters-same-workload` asserts equal request work, not sole cause. The section
        claimed only the backend differed while the configuration table above could be listing
        graph, memory or parallelism differences."""
        from collprof.core.compare import section_counters
        ...
        assert "only the backend differs" not in text
        assert "Equal request work is not sole-cause" in text
        assert "configuration section" in text

    def test_an_unclassified_entry_is_reported_but_not_differenced(self, tmp_path: Path):
        """A sysfs name the engine has not classified can be a gauge or a timer: differencing one
        reports its rise as traffic and drops its fall as a wrap."""
        from collprof.core.compare import build_counters, section_counters
        from collprof.core.rdma_counters import CounterSeries

        left = self.arm("MoRI", tmp_path, [2])
        right = self.arm("DeepEP", tmp_path, [2])
        for arm, value in ((left, 40), (right, 90)):
            series = arm.counters["decode_NODE2"]
            deltas = dict(series.deltas)
            deltas[("mlx5_0", "1", "active_rc_qp")] = value
            arm.counters["decode_NODE2"] = CounterSeries(
                node=series.node, seconds=series.seconds, samples=series.samples,
                adapters=series.adapters, deltas=deltas)

        kinds = {r[0] for r in build_counters(left, right, SPEC)[1]}
        text = "\n".join(section_counters(left, right, SPEC))

        assert "active_rc_qp" not in kinds, "a queue-pair gauge is not fabric traffic"
        assert "rx write req" in kinds, "the declared kinds are still compared"
        assert "Reported but not compared: active_rc_qp" in text
...
class TestPortQualifiers:
    """`mlx5_0:1` names a port. Dropping the suffix sampled every port of a multi-port card."""

    def sysfs(self, root: Path, ports: dict) -> Path:
        for port, value in ports.items():
            d = root / "mlx5_0" / "ports" / str(port) / "hw_counters"
            d.mkdir(parents=True, exist_ok=True)
            (d / "rx_write_req").write_text(f"{value}\n")
        return root

    def sampled_ports(self, out: Path) -> dict:
        rows = [ln.split(",") for ln in out.read_text().splitlines()[1:]]
        return {r[3]: int(r[5]) for r in rows}

    def run(self, root: Path, out: Path, *devices: str) -> subprocess.CompletedProcess:
        args = [a for d in devices for a in ("--devices", d)]
        return subprocess.run(["bash", str(SAMPLER), "--out", str(out), "--once", *args],
                              env={"PATH": "/usr/bin:/bin", "RDMA_SYSFS_ROOT": str(root)},
                              capture_output=True, text=True, timeout=60)

    def test_a_port_qualified_request_samples_only_that_port(self, tmp_path: Path):
        root = self.sysfs(tmp_path / "sys", {1: 100, 2: 200})
        out = tmp_path / "o.csv"

        assert self.run(root, out, "mlx5_0:1").returncode == 0

        assert self.sampled_ports(out) == {"1": 100}, "port 2 is not this run's"

    def test_a_bare_device_still_means_every_port(self, tmp_path: Path):
        root = self.sysfs(tmp_path / "sys", {1: 100, 2: 200})
        out = tmp_path / "o.csv"

        assert self.run(root, out, "mlx5_0").returncode == 0

        assert self.sampled_ports(out) == {"1": 100, "2": 200}

    def test_a_device_named_both_ways_keeps_the_wider_intent(self, tmp_path: Path):
        """The lists come from different transports; one naming the whole device means it may use
        any port, and narrowing to the other's port would drop traffic that is really there."""
        root = self.sysfs(tmp_path / "sys", {1: 100, 2: 200})
        out = tmp_path / "o.csv"

        assert self.run(root, out, "mlx5_0:1", "mlx5_0").returncode == 0

        assert self.sampled_ports(out) == {"1": 100, "2": 200}

    def test_a_requested_port_without_counters_is_a_failed_measurement(self, tmp_path: Path):
        """Validating the device alone passed `mlx5_0:2` whenever port 1 had counters; the sample
        then filtered port 1 out and exited 0 with a header only."""
        root = self.sysfs(tmp_path / "sys", {1: 100})

        done = self.run(root, tmp_path / "o.csv", "mlx5_0:2")

        assert done.returncode == 3
        assert "mlx5_0:2" in done.stderr, "the missing pair is named, not just the device"

    def test_the_exact_match_prefix_keeps_its_port(self, tmp_path: Path):
        root = self.sysfs(tmp_path / "sys", {1: 100, 2: 200})
        out = tmp_path / "o.csv"

        assert self.run(root, out, "=mlx5_0:2").returncode == 0

        assert self.sampled_ports(out) == {"2": 200}
...
class TestRequestedAdaptersAreChecked:
    ...
    def test_a_partial_set_is_refused_in_the_default_mode_too(self, tmp_path: Path):
        """Without `--devices` every discovered adapter is the requested set, so a mix of usable
        and counterless ones is the same partial total -- it used to warn and exit 0."""
        root = self.sysfs(tmp_path / "sys", with_counters=["mlx5_0"], without=["mlx5_9"])

        done = subprocess.run(["bash", str(SAMPLER), "--out", str(tmp_path / "o.csv"), "--once"],
                              env={"PATH": "/usr/bin:/bin", "RDMA_SYSFS_ROOT": str(root)},
                              capture_output=True, text=True, timeout=60)

        assert done.returncode == 3
        assert "partial set" in done.stderr
        assert "--devices" in done.stderr, "and it says how to name the subset deliberately"
