...
# the pid it holds.
#: Identifies this attempt. The barrier reads logs over a shared path that a requeue reuses, so
#: it has to tell a log this attempt truncated from one the last left behind. Not by timestamp:
#: the container's clock and this filesystem's are about 460 seconds apart here, the same offset
#: `resolve_traces` refuses to match across. `SLURM_RESTART_COUNT` is what changes on a requeue of
#: the same job. With neither forwarded the token is constant and the check degrades to what it
#: was before -- no worse, and the launcher says so.
_ATTEMPT_TOKEN="attempt=${SLURM_JOB_ID:-0}.${SLURM_RESTART_COUNT:-0}"
[[ -n "${SLURM_JOB_ID:-}" ]] || echo "[profile] NOTE: SLURM_JOB_ID absent; ${_ATTEMPT_TOKEN} " \
    "cannot distinguish a requeue from the attempt before it" >&2
...
_log_is_this_attempt() {
    [[ -f "$1" ]] || return 1
    grep -qF "${_ATTEMPT_TOKEN}" "$1" 2>/dev/null
}
